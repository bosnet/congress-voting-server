...
const getFrozenAccounts = async (address) => {
  const client = await loadHosts().then(newClient);
  const res = await client.get(`/api/v1/accounts/${address}/frozen-accounts?limit=100`);
...
const getTransaction = async (h) => {
  const client = await loadHosts().then(newClient);
  const res = await client.get(`/api/v1/transactions/${h}`);
...
const newProposal = async (opts) => {
  const {
    source,
    secret,
    sequence_id, // eslint-disable-line camelcase
    contract,
    start,
    end,
  ...
  if (account.sequence_id !== sequence_id) { // eslint-disable-line camelcase
    throw new Error('sequence_id mismatch');
  }
