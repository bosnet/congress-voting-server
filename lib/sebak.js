...
const newProposal = async (opts) => {
  const {
    source, secret, sequence_id, contract, start, end, funding_address, amount, // eslint-disable-line camelcase
  } = opts;
  const account = await getAccount(source);
  if (account.sequence_id !== sequence_id) {
...
const newVotingResult = async (opts) => {
  const { source, secret, sequence_id, operations } = opts;
  const account = await getAccount(source);
  if (account.sequence_id !== sequence_id) {
    throw new Error('sequence_id mismatch')
  }
