...
async function membershipDegradeTask() {
  const rawMemberships = await Membership.findActiveMembers();
  const members = rawMemberships.map(m => m.publicAddress);
  for (const addr of members) {
    const m = await Membership.findByAddress(addr);
    if (!m) { throw new Error('The address does not exist'); }
    const accounts = await getFrozenAccounts(addr);
...
membershipDegradeTask.$name = 'membership-degrade';
