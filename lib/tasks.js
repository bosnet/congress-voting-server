...
const { sequelize, Membership, Proposal, Vote } = require('../models/index');
const {
  newClient,
  loadHosts,
  newProposal,
...
async function confirmVotingResult(height, proposerAddress) {
  ...
  let records = res.body._embedded.records; // eslint-disable-line no-underscore-dangle
...
async function aggregatePF00CandidatesTask({ startDate }) {
  ...
  const activeRows = await sequelize.query(
    'SELECT ' +
      'members.id AS id, members."publicAddress" AS address, members.status AS status, log."createdAt" AS "createdAt" ' +
  ...
  const activeMembers = activeRows[1].rows.map((r) => {
    ...
    return [r.id, r.address, d.toISOString()];
  });

  // TODO: next time this should be re-checked if data is correct
  const deactiveRows = await sequelize.query(
    'SELECT ' +
      'log.id AS id, log."publicAddress" AS address, ' +
      'log."createdAt" AS "verifiedAt", log2."createdAt" AS "deletedAt" ' +
  ...
  const deactiveMembers = deactiveRows[1].rows.map((r) => {
    return [r.id, r.address, r.verifiedAt.toISOString(), r.deletedAt.toISOString()];
  });

  const result = activeMembers.concat(deactiveMembers);
  const csvData = result.map(m => m.slice(1).join(','));
  csvData.splice(0, 0, 'address,premembership,deleted');
  fs.writeFileSync('premebers.csv', csvData.join('\n'));
};
